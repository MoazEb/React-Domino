...
const C3 = () => {
    ...
    const handleInsertSimilar = (newCard, newCardReversed) => {
        ...
        if (userInput.toLowerCase() === 'l' && playedCards[0].count === newCard._const)
            insertLeft(newCard);
        else if (userInput.toLowerCase() === 'l' && playedCards[0].count === newCard.count)
    ...
    const insertToPlayedCards = (k) => {
        ...
        if (playedCards.length === 0)
            insertRight(newCard);
        else if (allowedNums.includes(+newCard._const) && allowedNums.includes(+newCard.count) && newCard._const !== newCard.count && allowedNums[0] !== allowedNums[1]) {
    ...
    return (
        <>
            {!matchEnded && (
                <>
                    ...
                    <div className={`relative hidden sm:flex h-screen w-full items-center justify-center flex-wrap`}>
                            ...
                            <AnimatePresence>
                                {player_1_cards.map((p, index) => (
                                    <motion.div
                                        ...
                                        onClick={() => playedCards.length > 0 && !(allowedNums.includes(p.count) || allowedNums.includes(p._const)) ? null : handleRole(p.key, index, 'p1')}
                            ...
                            <AnimatePresence>
                                {player_2_cards.map((p, index) => (
                                    <motion.div
                                        ...
                                        exit={{ opacity: 0, y: -20 }}
                                        transition={{ duration: 0.3 }}
                                        onClick={() => playedCards.length > 0 && !(allowedNums.includes(p.count) || allowedNums.includes(p._const)) ? null : handleRole(p.key, index, 'p2')}
                                    >
                                        <CardLayout
                                            _const={p._const}
                                            count={p.count}
                        ...
                        <motion.div
                            className={`absolute left-0 top-0 flex flex-col ${(disableOrNot()) ? 'pointer-events-none cursor-not-allowed bg-slate-200' : ''}  h-full overflow-y-auto`}
                            onClick={handleSwipe}
                        >
                            <AnimatePresence>
            ...
            {matchEnded && (player_1_cards.length === 0 || player_2_cards.length === 0) && <Winner player={player_1_cards.length === 0 ? 'Player 1' : 'Player 2'} />}
